...
| 姓名 | 学号 | 负责模块 | 主要工作内容 |
|------|------|---------|-------------|
| 骆深敏 | 2024611026 | A 算法模块 | 图数据结构设计、Kahn单条拓扑排序、DFS回溯全拓扑枚举、环检测与环路径定位、接口契约制定、算法模块文档统筹 |
| 易雨杰 | 2024611209 | B GUI与控制模块 | Swing主窗口三栏布局、MainController主控制器、结果面板分页显示、交互逻辑、模块对接联调、UI统一美化、项目统筹 |
| 戴燕岚 | 2024611180 | C 可视化模块 | GraphPanel节点和边绘制、分层/环形双布局、滚轮缩放拖拽平移、拓扑序颜色轮换高亮、节点序号标注、PNG图片导出 |
| 黄佳慧 | 2024611022 | D IO模块 | 输入数据解析与格式校验、错误行号定位、TXT/CSV排序结果导出、PNG导出接口、异常处理、边界场景测试 |
| 吴丽梅 | 2024611020 | E 测试与交付 | 全功能测试用例设计、测试报告、会议记录整理、中期汇报视频录制、用户手册编写、工程打包交付 |
