...
--------------------------
1. 将命令行/资源管理器定位到【项目根目录 topo-sort-app/】
2. 双击 run_tests.bat
...
--------------------------
# 第一步编译全部代码（在项目根目录 topo-sort-app/ 执行）
...
参数说明：
    第一个参数：输入图文本文件路径（相对路径，基于项目根目录topo-sort-app/）
...
打包后运行示例：
java -jar topo-sort-app.jar

============================================================
三、Linux / macOS 手动编译&运行
============================================================
# 编译（项目根目录 topo-sort-app/ 执行）
javac -encoding UTF-8 \
-sourcepath src:test \
-d bin \
src/model/*.java \
...
============================================================
❗关键：IDE运行时，请把【工作目录 Working directory 设置为项目根目录 topo-sort-app/】，
...
六、干净环境解压运行提示【验收重点】
============================================================
从提交压缩包解压之后，**必须进入解压出来的项目根目录执行全部编译/运行命令，不要进入src子文件夹执行**。
...
A：必须在【项目根目录 topo-sort-app/】启动程序，不要进入src子目录运行；IDE检查工作目录配置。
