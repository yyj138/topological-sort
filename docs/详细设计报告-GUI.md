...
本报告由组员 B 编写，对应 GUI 主框架与交互控制部分（任务 T-B1 ~ T-B8）的详细设计，属于项目 CST4823A 高级算法原理实践，文档版本 V1.0，编写日期 2026 年 9 月 17 日，指导教师廖海泳 / 陈银冬。报告描述界面布局、事件处理流程、GUI 类结构与调用关系，作为编码实现的直接依据，并以 9 月 17 日冻结的跨模块接口契约为约束基准。

## 一、设计概述

### 1.1 设计目标

本详细设计报告针对 GUI 主框架与交互控制部分（组员 B 任务 T-B1 ~ T-B8），描述界面布局、事件处理流程、GUI 类结构与调用关系，作为编码实现的直接依据。

### 1.2 设计原则

- **面向对象**：每个面板、按钮、控制器均为独立类，职责单一；
- **MVC 分层**：Model（Graph / 算法）— View（Swing 组件）— Controller（MainController 编排）；
- **配置集中**：UIStyle 工具类统一管理配色、字体、间距，便于整体美化与维护；
- **异常友好**：所有用户操作均经 try-catch 包装，错误中文提示 + 行号定位；
- **解耦桩**：B 模块依赖的 A/C/D 接口以桩类形式存在，便于独立编译运行与后续替换。

### 1.3 设计约束

- 运行环境：JDK 8+，零外部依赖；
- GUI 框架：Swing（保证现场验收可运行）；
- 字符编码：UTF-8（含中文注释与中文节点名不乱码）；
- 接口契约：以 9.17 冻结清单为准，本设计与之保持一致。
